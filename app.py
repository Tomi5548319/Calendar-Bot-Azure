...
def connect() -> bool:
    ...
    # time.
    if os.path.exists(get_file_route('token.json')):
        creds = Credentials.from_authorized_user_file(get_file_route('token.json'), SCOPES)
        ...
        if creds and creds.expired and creds.refresh_token:
            ...
            try:
                flow = InstalledAppFlow.from_client_secrets_file(
                    get_file_route('credentials.json'), SCOPES)
                creds = flow.run_local_server(port=0)
            except FileNotFoundError:
                print("No file credentials.json found")
                return False
        # Save the credentials for the next run
        with open(get_file_route('token.json'), 'w') as token:
